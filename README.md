...
└── eello/
    └── elpring/
        └── app/
            ├── Application.java             # 애플리케이션 실행 진입점 (main)
            ├── controller/
            │   └── TodoController.java      # REST API 컨트롤러 레이어
            ├── service/
            │   └── TodoService.java        # 비즈니스 로직 서비스 레이어
            ├── repository/
            │   ├── TodoRepository.java      # 리포지토리 인터페이스
            │   └── MemoryTodoRepository.java # 인메모리 구현체 레이어
            └── domain/
                ├── Todo.java               # Todo 도메인 모델 엔티티
                ├── TodoRequest.java        # 생성/수정 요청용 DTO
                └── TodoResponse.java       # 응답용 DTO
```
...
## 🧪 API 수동 검증 가이드 (CURL Command Examples)
